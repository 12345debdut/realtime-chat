...
- Tap search icon in header: *Future — in-chat-list search. Out of scope for v1.*

**Badge:** If unread messages exist, the Messages tab icon shows a numeric badge. *(v1: no unread count — deferred to v1.1.)*
...
- Log Out button
- App version
- *Future: notification preferences, theme toggle, blocked users list*
...
| `accepted` | Both users are connected. A DM room exists. | Terminal. Cannot be undone via the request system. *(Blocking/unfriending is a separate future feature.)* | None (terminal) |
...
| `connection:request:ignored` | Server --> Client | `{ requestId: string }` | Receiver ignores a request | *Not sent* (by design — sender should not know) |
...
*End of spec. All questions, edge case discoveries, or design feedback should be discussed in the #product channel before implementation begins.*
